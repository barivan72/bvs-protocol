const CACHE='rex-relax-2026-09-13-9';
const ASSETS=['/rex-relax/index.html','/rex-relax/manifest.webmanifest','/rex-relax/icon.svg','/rex-relax/audio-v5-data.js?v=20260913-9','/rex-relax/audio-v5.js?v=20260913-9'];
self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(ASSETS)).then(()=>self.skipWaiting())));
self.addEventListener('activate',e=>e.waitUntil((async()=>{await self.clients.claim();const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('rex-relax-')&&k!==CACHE).map(k=>caches.delete(k)));for(const c of await self.clients.matchAll({type:'window'}))c.postMessage({type:'REX_UPDATE_READY',release:'2026.09.13.9'});})()));
